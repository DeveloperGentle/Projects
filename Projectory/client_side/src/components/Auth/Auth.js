...
const signIn = async (formData, navigate,setUser) => {
...
const signUp = async (formData, navigate,setUser) => {
 try {
...
const onGoogleSuccess = (data,navigate) => {
 localStorage.setItem('profile',JSON.stringify(data))
 navigate('/all')
}
const Auth = ({setUser}) => {
...
if(isSignup) signUp(formData, navigate,setUser)
 
else signIn(formData, navigate,setUser)
...
try {
 onGoogleSuccess(result,navigate)
 navigate('/')
} catch (error) {
 console.log(error)
}
         ...
         <div className="flex-auto px-4 lg:px-10 py-10 pt-0">
          ...
          {/* { isSignup &&  <div className="mb-3">
             ...
             <div className="text-center mt-3">
               <button
                 className="bg-[#EC5252] text-white active:bg-blueGray-600 text-sm font-bold  px-6 py-3 rounded shadow hover:shadow-lg outline-none focus:outline-none mr-1 mb-1 w-full ease-linear transition-all duration-150"
                 type="submit"
               >
